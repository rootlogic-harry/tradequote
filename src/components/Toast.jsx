...
export default function Toast({ message, type = 'info', onDismiss }) {
  ...
  return (
    <div className={`fixed bottom-6 left-4 right-4 sm:left-auto sm:right-6 z-50 border rounded-lg px-4 py-3 shadow-lg max-w-sm text-sm font-body animate-[slideUp_0.3s_ease-out] flex items-center justify-between gap-2 ${TYPE_STYLES[type] || TYPE_STYLES.info}`}>
      <span>{message}</span>
      {type === 'error' && (
