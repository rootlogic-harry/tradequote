 ...
 *
 * When `videoEnabled` is false the video card is hidden entirely and
 * the screen collapses to a single-card layout. This is the
 * production fail-state while the video pipeline rebuild is in
 * flight — see docs/VIDEO_FLAG.md.
 *
 * @param {object} props
 * @param {(mode: 'video' | 'photos') => void} props.onSelectMode
 * @param {'video' | 'photos' | null} [props.defaultMode]
 * @param {boolean} [props.videoEnabled=true]
 */
export default function CaptureChoice({ onSelectMode, defaultMode = null, videoEnabled = true }) {
  return (
    <div style={{
      display: 'flex',
