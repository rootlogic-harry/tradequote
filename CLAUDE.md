...
| AI Agents | Claude Opus 5 (`claude-opus-5`) — self-critique, feedback, calibration. Upgraded from Haiku 4.5 on 2026-09-18. |
...
- **Billing (TRQ-150 + 2026-06-24 pack + 2026-06-30 invoices)**: `GET /api/billing/status` exposes `quotaState`, `freeQuotesUsed/Limit`, `purchasedQuotesRemaining`, and a `quotePack` pricing block. `POST /api/billing/checkout` opens a Stripe Checkout for the £19.99/month subscription. `POST /api/billing/buy-quote-pack` opens a Stripe Checkout for the £9.99 one-time pack (`mode: 'payment'`, `automatic_tax: false`, `invoice_creation: { enabled: true }` — Stripe auto-creates a hosted invoice the user can download from their Billing section). `POST /api/billing/portal` opens the Stripe billing portal for self-serve subscription management. `GET /api/billing/purchases` (auth-gated, rate-limited) returns the merged history: `quote_purchases` rows (decorated with their `hosted_invoice_url`) plus the user's subscription invoices fetched live via `stripe.invoices.list`. `POST /api/billing/webhook` (mounted BEFORE `express.json`, raw body for HMAC) fans out to both `applySubscriptionEventToDb` and `applyQuotePackEventToDb` — disjoint routing, idempotent on `quote_purchases.stripe_payment_id` UNIQUE. Pack purchases capture `hosted_invoice_url` on the webhook write (best-effort; failure to fetch the URL never blocks the credit).
...
- **`public/sitemap.xml`** — thirteen URLs today: `/`, `/guides/`, the 8
  individual `/guides/*` cluster URLs (cost-per-metre, how-long-to-rebuild,
  stone-tonnage, whats-in-a-quote, dswa-day-rate, yorkshire-walling-costs,
  cotswold-walling-costs, chapter-8-traffic-management), plus `/privacy`,
  `/terms`, `/dpa`. Adding a new guide means adding the URL here AND the
  markdown file under `content/guides/`. Asserted by
  `discoverability.test.js`'s "contains all 8 guide URLs" guard.
...
| `users.purchased_quotes`, `quote_purchases` (2026-06-24, `hosted_invoice_url` added 2026-06-30) | Pay-as-you-go pack — £9.99 for 5 quotes, no expiry. `purchased_quotes` is the user's current balance (decremented atomically on a successful analyse when the gate's reason was `purchased-remaining`). `quote_purchases` is the audit row per Stripe PaymentIntent — `stripe_payment_id UNIQUE` makes the webhook idempotent on redelivery. The `hosted_invoice_url` column (nullable; pre-2026-06-30 rows are NULL — see PR #103) carries Stripe's hosted-invoice page URL for the Settings → Billing download link. Refund handling is manual — see `docs/REFUNDS.md`. |
...
| `users.signup_source`, `signup_campaign`, `signup_medium` (2026-08-04, ad-attribution) | Three nullable TEXT columns capturing `?utm_source` / `?utm_campaign` / `?utm_medium` at signup for the £100 Meta ad test (docs/AD_TEST spec 2026-07-07). NULL = direct / organic / word-of-mouth. Session-stashed via `stashPendingUtm(req)` at all four entry points (`/`, `/signup`, `/login`, `/auth/login`) — first-write-wins so a landing capture is never overwritten. Lifted before `req.session.regenerate()` in `/auth/callback` and applied via `applyUtmAtSignup(userId, utm)` — a single `UPDATE users` guarded by `_isNewUser` so returning users never have their original attribution overwritten. Coexists with `?ref=` — a Meta ad viewer who ALSO carries a referral code gets BOTH written (ref governs the bonus-quote reward, UTM governs analytics). Zero effect on quota / rewards / billing — analytics only. **Admin surface (PR 2, 2026-08-04)**: `Analytics.jsx` has a "Signups by source" section above per-user spend that shows `Source \| Signups \| Activated \| Paying` per distinct `signup_source` (NULL bucketed as "direct"). "Activated" = ≥1 `quote_analysed` event on record. "Paying" = `subscription_status='active'` OR `purchased_quotes>0`. The per-user table also gains a Source column between Plan and Last login. Powered by `signupsBySourceQuery` in `/api/admin/analytics`, projected into `data.signupsBySource: [{source,signups,activated,paying}]`. |
...
8. **Smoke suite runs on every PR.** `npm run smoke` (Playwright, `tests/e2e/`) covers public surfaces + planned auth-gated journeys. It's the "did the user-visible behaviour break?" gate that source-level tests can't provide. New user-facing routes / status transitions / redirects need at least one smoke assertion. See `docs/SMOKE.md` for the runbook.
